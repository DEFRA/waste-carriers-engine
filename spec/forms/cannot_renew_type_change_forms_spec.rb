...
RSpec.describe CannotRenewTypeChangeForm, type: :model do
  describe "#submit" do
    context "when the form is valid" do
      let(:cannot_renew_type_change_form) { build(:cannot_renew_type_change_form, :has_required_data) }
      let(:valid_params) { { reg_identifier: cannot_renew_type_change_form.reg_identifier } }

      it "should submit" do
        expect(cannot_renew_type_change_form.submit(valid_params)).to eq(true)
      end
    end
    ...
    context "when the form is not valid" do
      ...
      it "should not submit" do
        expect(cannot_renew_type_change_form.submit(invalid_params)).to eq(false)
      end
    end
  end

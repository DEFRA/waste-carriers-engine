  ...
  RSpec.describe RegistrationNumberForm, type: :model do
    ...
    describe "#submit" do
      context "when the form is valid" do
        ...
        let(:valid_params) { { _id: registration_number_form._id, company_no: "09360070" } }

        it "should submit" do
          expect(registration_number_form.submit(valid_params)).to eq(true)
        end

        context "when the _id is less than 8 characters" do
      ...
      context "when the form is not valid" do
        let(:registration_number_form) { build(:registration_number_form, :has_required_data) }
        let(:invalid_params) { { _id: "foo", company_no: "foo" } }

...
module WasteCarriersEngine
  RSpec.describe NewRegistration do
    subject { build(:new_registration, workflow_state: "company_trading_name_form") }

    describe "#workflow_state" do
      context ":company_trading_name_form state transitions" do
        context "on next" do
          context "when the business is based overseas" do
            subject { build(:new_registration, workflow_state: "company_trading_name_form", location: "overseas") }
        ...
        context "on back" do
          context "when the business does not require a registration number and the registration is upper tier" do
            subject { build(:new_registration, :upper, workflow_state: "company_trading_name_form", location: "overseas") }

            include_examples "has back transition", previous_state: "cbd_type_form"
          end

          context "when the registration is lower tier" do
            subject { build(:new_registration, :lower, workflow_state: "company_trading_name_form") }

            context "when the check your tier answer is lower" do
              subject { build(:new_registration, :lower, workflow_state: "company_trading_name_form", temp_check_your_tier: "lower") }
          ...
          context "When the registration requires a company number" do
            subject { build(:new_registration, workflow_state: "company_trading_name_form", business_type: "limitedCompany") }

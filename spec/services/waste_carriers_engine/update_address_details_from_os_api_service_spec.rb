...
module WasteCarriersEngine
  RSpec.describe UpdateAddressDetailsFromOsApiService, type: :service do
    ...
    describe "#run" do
      ...
      context "when given a valid postcode" do
        ...
        it { expect(address.dependent_locality).to be_nil }
        ...
        it { expect(address.country).to eq "United Kingdom" }
        it { expect(address.local_authority_update_date).to eq "10/02/2016" }
        it { expect(address.easting).to eq(358_205) }
        it { expect(address.northing).to eq(172_708) }
      end
    end
  end
end

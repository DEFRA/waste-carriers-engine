...
module WasteCarriersEngine
  RSpec.describe ValidateGovpayWebhookBodyService do
    describe ".run" do
      ...
      shared_examples "fails validation" do
        it "raises an exception" do
          expect { run_service }.to raise_error(ValidateGovpayWebhookBodyService::ValidationFailure)
        end

        it "logs an error" do
          run_service
          expect(Airbrake).to have_received(:notify)
        rescue ValidateGovpayWebhookBodyService::ValidationFailure

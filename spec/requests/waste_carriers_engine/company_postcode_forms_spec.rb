      ...
      context "when a valid user is signed in" do
        ...
        context "when a valid transient registration exists" do
          ...
          context "when the back action is triggered" do
            it "returns a 302 response and redirects to the business_name form" do
              get back_company_postcode_forms_path(transient_registration[:token])

              expect(response).to have_http_status(302)
              expect(response).to redirect_to(new_business_name_form_path(transient_registration[:token]))
            end
          end
        end

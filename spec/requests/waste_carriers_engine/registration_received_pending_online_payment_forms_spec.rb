...
module WasteCarriersEngine
  RSpec.describe "RegistrationReceivedPendingOnlinePaymentForm", type: :request do
    describe "GET new_registration_received_pending_online_payment_form_path" do
      context "when no new registration exists" do
        it "redirects to the invalid page" do
          get new_registration_received_pending_online_payment_form_path("wibblewobblejellyonaplate")
      ...
      context "when a valid new registration exists" do
        let(:transient_registration) do
          create(
            :new_registration,
            :has_required_data,
            workflow_state: "registration_received_pending_online_payment_form"
          )
        end
        ...
        context "when the workflow_state is correct" do
          it "returns a 200 status, renders the :new template, creates a new registration and deletes the transient registration" do
            ...
            get new_registration_received_pending_online_payment_form_path(transient_registration.token)

  ...
  RSpec.describe "YourTierForm", type: :request do
    ...
    describe "POST your_tier_form_path" do
      ...
      context "when the new registration is a lower tier registration" do
        ...
        it "updates the transient registration workflow and redirects to the company_trading_name_form with a 302 status code" do
          post new_your_tier_form_path(params)

          new_registration.reload

          expect(response).to redirect_to(new_company_trading_name_form_path(new_registration.token))
          expect(response).to have_http_status(302)
          expect(new_registration.workflow_state).to eq("company_trading_name_form")
        end
      end

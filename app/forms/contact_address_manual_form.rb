class ContactAddressManualForm < BaseForm
  ...
  def initialize(transient_registration)
    ...
    # Prefill the existing address unless the temp_contact_postcode has changed from the saved postcode
    # Otherwise, just fill in the temp_contact_postcode
    saved_address_still_valid? ? prefill_existing_address : self.postcode = @transient_registration.temp_contact_postcode
  end
  ...
  def saved_address_still_valid?
    ...
    return true if @transient_registration.temp_contact_postcode == @transient_registration.registered_address.postcode
    false
  end
  ...
  def add_or_replace_address(params)
    ...
    # Update the transient object's nested addresses, replacing any existing contact address
